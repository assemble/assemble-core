...
- Bumped `templates` to latest, 0.6.0. templates 0.6.0 uses the latest [base-methods][], which introduces prototype mixins. No API changes.
...
{%= reflinks(verb.related.list.concat(Object.keys(dependencies)).concat(['gulp', 'vinyl-fs', 'less', 'handlebars', 'assemble-cli', 'assemble-streams', 'emitter-only', 'base-methods'])) %}
